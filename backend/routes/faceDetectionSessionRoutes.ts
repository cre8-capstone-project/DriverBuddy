...
const faceDetectionSessionRoutes = (
  ...
  router.post('/register', async (req, res) => {
    try {
      ...
      const alertsTimestamp: admin.firestore.Timestamp[] = alerts
        ? alerts.map((alert: string) => admin.firestore.Timestamp.fromDate(new Date(alert)))
        : [];
